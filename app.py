...
@st.cache_data
def load_dataset_from_file(_file_bytes: bytes):
    import io
    return pd.read_excel(io.BytesIO(_file_bytes))
...
uploaded_file = st.sidebar.file_uploader(
    "Upload teff.xlsx",
    type=["xlsx", "xls"],
    help="Upload your teff dataset to view data overview and EDA.",
)
...
if uploaded_file is not None:
    df_raw = load_dataset_from_file(uploaded_file.getvalue())
    st.sidebar.success(f"Loaded: {uploaded_file.name}")
else:
    df_raw = load_dataset_from_disk()
    if df_raw is not None:
        st.sidebar.info("Using teff.xlsx from repo")
...
if page == "Home":
    ...
    if df is None:
        st.warning("Upload `teff.xlsx` using the sidebar uploader, or add it to the repo root / `data/` folder.")
    else:
    ...
    if df is None:
        st.warning("Dataset not found. Upload `teff.xlsx` in the sidebar for EDA visualizations.")
    else:
